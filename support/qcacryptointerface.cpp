...
class QCACryptoInterface::Private {
public:
    Private()
    {
...
QCACryptoInterface::QCACryptoInterface()
{
    p = new Private;
}
...
QByteArray QCACryptoInterface::generateSalt(const QString &value)
{
    return QCA::InitializationVector(value.toLatin1()).toByteArray();
}

QByteArray QCACryptoInterface::generateInitalizationVector(int size)
{
    return QCA::InitializationVector(size).toByteArray();
}

SecureArray QCACryptoInterface::generateSymmetricKey(int size)
{
    return QCA::SymmetricKey(size).toByteArray();
}

QString QCACryptoInterface::generateUid()
{
...
WP::err QCACryptoInterface::encryptSymmetric(const SecureArray &input, QByteArray &encrypted,
...
WP::err QCACryptoInterface::decryptSymmetric(const QByteArray &input, SecureArray &decrypted,
                                      const SecureArray &key, const QByteArray &iv,
                                      const char *algo)
{
...
QByteArray QCACryptoInterface::sha1Hash(const QByteArray &string) const
{
    QCA::Hash shaHash("sha1");
...
QString QCACryptoInterface::toHex(const QByteArray &string) const
{
